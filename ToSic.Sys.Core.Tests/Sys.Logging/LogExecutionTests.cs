...
public class LogExecutionTests
{
    ...
    [Fact]
    public void LinkedLogs_KeepTheActiveOperationTree()
    {
        var root = new Log("Tst.Root");
        var first = new Log("Tst.First", root);
        var second = new Log("Tst.Second", root);

        var parent = root.Fn("parent");
        var child = first.Fn("child");
        var grandchild = second.Fn("grandchild");
        grandchild.Done();
        child.Done();
        parent.Done();

        var entries = LogSnapshot.FromLegacy(root).Entries;
        Equal(parent.Entry!.Sequence, Single(entries, e => e.Sequence == child.Entry!.Sequence).ParentOperationId);
        Equal(child.Entry!.Sequence, Single(entries, e => e.Sequence == grandchild.Entry!.Sequence).ParentOperationId);
    }

    [Fact]
    public void LinkedLogs_FollowNewestOperation_AfterIntermediateLoggerIsRelinked()
    {
        var root = new Log("Tst.Root");
        var middle = new Log("Tst.Middle");
        var leaf = new Log("Tst.Leaf", middle);
        middle.LinkTo(root);

        var rootCall = root.Fn("root");
        var completed = leaf.Fn("completed");
        completed.Done();

        var activeParent = middle.Fn("active");
        var nested = leaf.Fn("nested");

        var entries = LogSnapshot.FromLegacy(root).Entries;
        Equal(activeParent.Entry!.Sequence,
            Single(entries, e => e.Sequence == nested.Entry!.Sequence).ParentOperationId);
    ...
    [Fact]
    public async Task LinkedLogs_IsolateActiveOperationsAcrossParallelFlows()
    {
        var root = new Log("Tst.Root");
        var left = new Log("Tst.Left", root);
        var right = new Log("Tst.Right", root);
        var parent = root.Fn("parent");

        await Task.WhenAll(Call(left, "left"), Call(right, "right"));
        parent.Done();

        var entries = LogSnapshot.FromLegacy(root).Entries;
        Equal(parent.Entry!.Sequence, Single(entries, e => e.Result == "left").ParentOperationId);
        Equal(parent.Entry.Sequence, Single(entries, e => e.Result == "right").ParentOperationId);

        static async Task Call(Log log, string name)
        {
            await Task.Yield();
    ...
    [Fact]
    public void LinkedLogs_RestoreOperations_AfterNormalAndOutOfOrderCompletion()
    {
        var root = new Log("Tst.Root");
        var first = new Log("Tst.First", root);
        var second = new Log("Tst.Second", root);
        ...
        var child = first.Fn("child");
        child.Done();
        var restored = second.Fn("restored");
        restored.Done();

        var stillOpen = first.Fn("still open");
        parent.Done();
        var nested = second.Fn("nested");
        nested.Done();
        stillOpen.Done();

        var after = root.Fn("after");
        after.Done();

        var entries = LogSnapshot.FromLegacy(root).Entries;
        Equal(parent.Entry!.Sequence, Single(entries, e => e.Sequence == restored.Entry!.Sequence).ParentOperationId);
        Equal(stillOpen.Entry!.Sequence, Single(entries, e => e.Sequence == nested.Entry!.Sequence).ParentOperationId);
        Null(Single(entries, e => e.Sequence == after.Entry!.Sequence).ParentOperationId);
    }
