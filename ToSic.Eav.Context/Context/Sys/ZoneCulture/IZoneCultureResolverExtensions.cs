...
public static class IZoneCultureResolverExtensions
{
    ...
    public static string?[] SafeLanguagePriorityCodes(this IZoneCultureResolver? resolver)
    {
        ...
        // ReSharper disable once SuspiciousTypeConversion.Global - reason is that DNN implements this
        var list = (resolver as IZoneCultureResolverProWIP)
    ...
    {
        if (resolver == null)
            throw new ArgumentNullException(nameof(resolver), @"this method is very internal and should never be called with non-existing resolvers");
